...
export function PetSelect({
  ...
  const handleSelect = (pet: IPetResponse) => {
    onChange(String(pet.id));
    setIsOpen(false);
  };
    ...
    <div className="flex flex-col gap-1.5" ref={containerRef}>
      ...
      <div className="relative">
        ...
        {selected && !disabled ? (
          <button
            type="button"
            aria-label={`Quitar a ${selectedName} como mascota`}
        ...
        {isOpen ? (
          ...
          >
            {isLoadingPets ? (
              <div className="px-3.5 py-3 text-[14px] text-[#526D82]">Cargando mascotas...</div>
            ) : petsError ? (
              <div className="px-3.5 py-3 text-[14px] text-[#c0392b]">{petsError}</div>
            ) : !hasSearch ? (
              ...
              <>
                {visiblePets.map((pet) => {
                  const isSelected = String(pet.id) === value;
                  ...
                  <button
                    ...
                    >
                      <div className="min-w-0 flex-1">
                        ...
                        <div className="truncate text-[12px] text-[#526D82]">{pet.breed || 'Sin raza'}</div>
                      </div>
                      {isSelected ? (
      ...
      {!error && selected ? (
        <span className="text-[12px] leading-4 text-[#526D82]">Mascota: {selectedName}</span>
      ) : null}
    </div>
  );
}
