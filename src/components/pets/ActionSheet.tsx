      ...
      <div className="absolute inset-x-0 bottom-0 z-41 rounded-t-[22px] bg-[#DDE6ED] px-[18px] pb-[34px] pt-5 shadow-[0_-8px_24px_rgba(39,55,77,0.25)]">
        ...
        <div className="mb-4 flex items-center gap-3">
          <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-[#27374D]">
            <span className="text-[16px] font-semibold text-white">{initials(selected.name)}</span>
          </div>
          <div className="min-w-0 flex-1">
            <div className="text-[15px] font-semibold text-[#27374D]">{selected.name}</div>
            <div className="truncate text-[13px] text-[#526D82]">{selected.specie} · {selected.breed}</div>
          </div>
        </div>
